...
onAuthStateChanged(auth, async user => {
  if (user) {
    ...
    // XAMChat-сессия важнее Firebase signOut
    if (window.hasXamSession?.()) { hideSplash(); return; }
...
// ============================================================
// ИСПРАВЛЕННЫЙ ОБРАБОТЧИК ПОИСКА (контакты + сообщества)
// Переопределяем, чтобы убрать вызов joinAndOpenRoom
// ============================================================
$('contacts-search').oninput = async function () {
  ...
  usersCache.forEach((u, uid) => {
    ...
    if (nm.includes(q) || un.includes(q)) {
      foundUsers.push({ uid, ...u });
    }
  });

  // 2. Поиск сообществ (группы и каналы)
  const foundRooms = [];
  roomsCache.forEach((r, id) => {
    const nm = (r.name || '').toLowerCase();
    const un = (r.username || '').toLowerCase();
    if (nm.includes(q) || un.includes(q)) {
      foundRooms.push({ id, ...r });
    }
  });

  if (!foundUsers.length && !foundRooms.length) {
    res.innerHTML = '<div style="padding:16px;text-align:center;color:var(--text-hint);font-size:13px;">Ничего не найдено</div>';
    return;
  }
  ...
  if (foundUsers.length) {
    html += '<div class="settings-label">Пользователи</div>';
    foundUsers.slice(0, 10).forEach(u => {
      const inCts = ctcs.includes(u.uid);
      html += `
        <div class="contact-item" data-uid="${u.uid}">
          <div class="ci-av">${avatarHtml(u.avatar)}</div>
          <div class="ci-info">
            <div class="ci-name">${esc(getDisplayName(u.uid))}</div>
            <div class="ci-uname">${esc(u.username || u.uid.slice(0,12))}</div>
          </div>
          <div class="ci-actions">
            <button class="ci-btn" data-uid="${u.uid}" data-action="msg">💬</button>
            <button class="ci-btn ${inCts ? 'red' : ''}" data-uid="${u.uid}" data-action="${inCts ? 'rem' : 'add'}">${inCts ? '✕' : '+'}</button>
          </div>
        </div>`;
    });
  }

  // --- Сообщества (все данные в data-атрибутах, чтобы не зависеть от кэша) ---
  if (foundRooms.length) {
    html += '<div class="settings-label">Сообщества</div>';
    foundRooms.slice(0, 10).forEach(r => {
      const isMember = r.members?.includes(me.uid) || r.subscribers?.includes(me.uid);
      const badge = r.type === 'channel' ? '📢 Канал' : '👥 Группа';
      html += `
        <div class="contact-item" 
             data-room-id="${r.id}" 
             data-room-name="${esc(r.name)}" 
             data-room-avatar="${esc(r.avatar || (r.type === 'channel' ? '📢' : '👥'))}" 
             data-room-type="${esc(r.type || 'group')}"
             data-is-member="${isMember}">
          <div class="ci-av">${avatarHtml(r.avatar || (r.type === 'channel' ? '📢' : '👥'))}</div>
          <div class="ci-info">
            <div class="ci-name">${esc(r.name)}</div>
            <div class="ci-uname">${esc(r.username || '')} <span style="opacity:0.6;">• ${badge}</span></div>
          </div>
          <div class="ci-actions">
            <button class="ci-btn ${isMember ? '' : 'green'}" data-action="room-join">${isMember ? '💬' : '+'}</button>
          </div>
        </div>`;
    });
  }
  ...
  res.querySelectorAll('.contact-item[data-uid]').forEach(div => {
    const uid = div.dataset.uid;
    div.querySelectorAll('.ci-btn').forEach(b => b.onclick = async e => {
      e.stopPropagation();
      const action = b.dataset.action;
      if (action === 'msg') {
        const chat = chatList.find(c => c.id === uid && c.type === 'private') || 
                     { id: uid, name: getDisplayName(uid), avatar: usersCache.get(uid)?.avatar, type: 'private' };
        openChat(chat);
      } else if (action === 'add') {
    ...
    div.onclick = e => { if (!e.target.closest('.ci-btn')) openUserView(uid); };
  });

  // --- Обработчики для сообществ (исправлено: нет вызова joinAndOpenRoom) ---
  res.querySelectorAll('.contact-item[data-room-id]').forEach(div => {
    const roomId = div.dataset.roomId;
    const roomName = div.dataset.roomName;
    const roomAvatar = div.dataset.roomAvatar;
    const roomType = div.dataset.roomType;
    let isMember = div.dataset.isMember === 'true';

    const btn = div.querySelector('.ci-btn');
    if (!btn) return;
    ...
    btn.onclick = async e => {
      ...
      if (!isMember) {
        try {
          const field = roomType === 'channel' ? 'subscribers' : 'members';
          await updateDoc(doc(db, 'rooms', roomId), { [field]: arrayUnion(me.uid) });
          isMember = true;
          div.dataset.isMember = 'true';
          btn.textContent = '💬';
          btn.classList.remove('green');
          showToast(`✅ Вы вступили в "${roomName}"`);
        } catch (error) {
          console.error('Ошибка вступления:', error);
          showToast('❌ Не удалось вступить');
          return;
        }
      }

      // Открываем чат (группу или канал)
      openChat({
        id: roomId,
        name: roomName,
        avatar: roomAvatar,
        type: roomType,
        roomData: { id: roomId, name: roomName, avatar: roomAvatar, type: roomType }
      });
    };

    // Клик по всей карточке (но не по кнопке)
    div.onclick = e => {
...
function renderSettings() {
  ...
  $('s-logout').onclick = () => { if (confirm('Выйти из аккаунта?')) (window.logoutNexLink || (() => signOut(auth)))(); };
}
...
(function() {
  ...
  const newAuthStateHandler = async (user) => {
    if (user) {
      ...
      // Не сбрасываем UI, если активна сессия XAMChat
      if (window.hasXamSession?.()) return;
...
// ============================================================
//  ПРАВИЛЬНАЯ УСТАНОВКА СЛУШАТЕЛЯ АУТЕНТИФИКАЦИИ
// ============================================================

// Удаляем все старые обработчики, сохраняя ссылки на них
...
window._authUnsubscribe = auth.onAuthStateChanged(async (user) => {
  // Ваша логика обработки состояния аутентификации
  // (здесь можно поместить содержимое newAuthStateHandler)
  if (user) {
    // ... обработайте вход
  } else {
    // ... обработайте выход
...
// Если пользователь уже залогинен, вызываем обработчик вручную
if (auth.currentUser) {
  // Вызываем через микротаск, чтобы избежать конфликтов
  Promise.resolve().then(() => {
    // Здесь можно вызвать ваш обработчик, но поскольку он уже будет
    // вызван через onAuthStateChanged, дополнительный вызов не нужен.
    // Если всё же нужно, используйте:
...
// ════════════════════════════════════════════════════════════════
//  XAMChat OAuth (Authorization Code + PKCE)
// ════════════════════════════════════════════════════════════════
(function initXamChatAuth() {
  'use strict';

  const XAM_CLIENT_ID = 'xam_UwnNYN6PTFm78wIhYqmQ8g';
  const XAM_REDIRECT_URI = `${location.origin}/`;
  const XAM_SESSION_KEY = 'nexlink_xam_session';
  const XAM_LOGO_SVG = `
    <svg class="xam-logo" viewBox="0 0 269 233" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <rect x="111.94" y="105.027" width="21.0128" height="150.315" rx="10.5064" transform="rotate(41.1383 111.94 105.027)" fill="currentColor"/>
      <rect x="253.017" y="1.27454" width="21.0128" height="162.327" rx="10.5064" transform="rotate(43.97 253.017 1.27454)" fill="currentColor"/>
      <rect x="140.561" y="118.032" width="22.1806" height="151.002" rx="11.0903" transform="rotate(-40.4969 140.561 118.032)" fill="currentColor"/>
      <rect y="15.3319" width="22.1806" height="161.745" rx="11.0903" transform="rotate(-43.7275 0 15.3319)" fill="currentColor"/>
      <rect x="2.7981" y="21.1798" width="22.1806" height="198.343" rx="11.0903" transform="rotate(-65.3761 2.7981 21.1798)" fill="currentColor"/>
      <rect x="78.6417" y="151.945" width="22.1806" height="184.527" rx="11.0903" transform="rotate(-64.3736 78.6417 151.945)" fill="currentColor"/>
    </svg>`;

  const style = document.createElement('style');
  style.id = 'xam-auth-panel-styles';
  style.textContent = `
    #auth-screen {
    ...
    #auth-screen.gone .xam-auth-panel,
    .xam-auth-panel.is-hidden {
      opacity: 0;
      visibility: hidden;
      pointer-events: none;
    }
    .xam-login-btn {
      ...
      cursor: pointer;
      color: #111318;
      font: 700 15px/1 var(--font, inherit);
      letter-spacing: .01em;
      background: #ffffff;
      box-shadow: none;
      transition: transform .12s ease, opacity .15s ease, background .15s ease, border-color .15s ease;
    }
    .xam-login-btn:hover:not(:disabled) {
      background: #ffffff;
      border-color: #1E56E8;
    }
    .xam-login-btn:active:not(:disabled) { transform: scale(.98); }
    .xam-login-btn:disabled { opacity: .65; cursor: wait; }
    .xam-login-btn .xam-logo {
      width: 26px;
      height: 22px;
      flex-shrink: 0;
      display: block;
  ...
  function saveXamSession(data, user) {
    const payload = {
      access_token: data.access_token,
      expires_at: Date.now() + (Number(data.expires_in) || 3600) * 1000,
      user
    };
    try { localStorage.setItem(XAM_SESSION_KEY, JSON.stringify(payload)); } catch (_) {}
    return payload;
  }

  function loadXamSession() {
    try {
      const raw = localStorage.getItem(XAM_SESSION_KEY);
      if (!raw) return null;
      const data = JSON.parse(raw);
      if (!data?.access_token || !data?.user?.id) return null;
      if (data.expires_at && Date.now() > data.expires_at) {
        localStorage.removeItem(XAM_SESSION_KEY);
        return null;
      }
      return data;
    } catch (_) {
  ...
  function clearXamSession() {
    try { localStorage.removeItem(XAM_SESSION_KEY); } catch (_) {}
  }

  window.hasXamSession = () => !!loadXamSession();

  function ensureRealtimeSubs() {
    if (window._nexlinkSubsStarted) return;
    window._nexlinkSubsStarted = true;
    onSnapshot(collection(db, 'users'), () => { refreshUsersCache(); buildChatList(); });
    onSnapshot(collection(db, 'rooms'), () => { refreshRoomsCache(); buildChatList(); });
  }

  async function enterAppWithProfile(uid, profile) {
    me = { uid, email: profile.email || null };
    myProfile = profile;
    usersCache.set(uid, profile);
    initialized = true;
    const authScreen = document.getElementById('auth-screen');
    const app = document.getElementById('app');
    if (authScreen) authScreen.classList.add('gone');
    if (app) app.classList.remove('hidden');
    document.querySelector('.xam-auth-panel')?.classList.add('is-hidden');
    hideSplash();
    await refreshCaches();
    await buildChatList();
    startHeartbeat();
  ...
  function xamUid(user) {
    return `xam_${String(user.id)}`;
  }

  function buildProfileFromXam(user) {
    const unameRaw = String(user.username || `xam_${user.id}`).replace(/^@/, '');
    const name = String(user.name || unameRaw || 'XAMChat User').trim();
  ...
  async function loginWithXamData(data) {
    let user = data.user;
    if (!user?.id && data.access_token && window.XamAuth?.fetchUser) {
      user = await XamAuth.fetchUser(data.access_token);
    }
    if (!user?.id) throw new Error('Не удалось получить профиль XAMChat');

    saveXamSession(data, user);
    const uid = xamUid(user);
    let profile = await getUserDoc(uid);
    if (!profile) {
      profile = buildProfileFromXam(user);
      ...
      const patch = {
        ...
        avatar: user.avatar_url || profile.avatar,
        username: profile.username || buildProfileFromXam(user).username,
        provider: 'xamchat',
        xamId: String(user.id)
      };
      if (user.name) {
        const parts = String(user.name).trim().split(/\s+/);
        patch.firstName = parts[0] || profile.firstName;
        patch.lastName = parts.slice(1).join(' ') || profile.lastName || '';
      }
      await updateDoc(doc(db, 'users', uid), patch).catch(() => setDoc(doc(db, 'users', uid), { ...profile, ...patch }, { merge: true }));
      profile = { ...profile, ...patch };
    }

    // Сбрасываем Firebase-сессию, чтобы не мешала verify/email flow
    try { if (auth.currentUser) await signOut(auth); } catch (_) {}

    await enterAppWithProfile(uid, profile);
    showToast(`👋 Привет, ${profile.name || profile.username}!`);
  }

  window.logoutNexLink = async function logoutNexLink() {
    clearXamSession();
    stopAllSubs();
    stopHeartbeat();
    window._nexlinkSubsStarted = false;
    initialized = false;
    me = null;
    myProfile = null;
    ...
    document.querySelector('.xam-auth-panel')?.classList.remove('is-hidden');
  ...
  function syncPanelVisibility() {
    const panel = document.querySelector('.xam-auth-panel');
    if (!panel) return;
    const authScreen = document.getElementById('auth-screen');
    const app = document.getElementById('app');
    const appVisible = app && !app.classList.contains('hidden');
    const authGone = authScreen && authScreen.classList.contains('gone');
    panel.classList.toggle('is-hidden', !!(appVisible || authGone || me));
  }

  function mountPanel() {
    if (document.querySelector('.xam-auth-panel')) return;

    const panel = document.createElement('div');
    panel.className = 'xam-auth-panel';
    panel.innerHTML = `
      <button type="button" class="xam-login-btn" id="xam-login-btn">
        ${XAM_LOGO_SVG}
        <span>Войти через XAMChat</span>
      </button>`;

    const authScreen = document.getElementById('auth-screen');
    const authCard = authScreen?.querySelector('.auth-card');
    if (authCard) authCard.insertAdjacentElement('afterend', panel);
    else if (authScreen) authScreen.appendChild(panel);
    else document.body.appendChild(panel);

    const btn = panel.querySelector('#xam-login-btn');
    btn.addEventListener('click', async () => {
      if (!window.XamAuth?.signIn) {
        showToast('XAM Auth SDK не загружен');
        return;
      }
      btn.disabled = true;
      const label = btn.querySelector('span');
      const prev = label.textContent;
      label.textContent = 'Открываем вход…';
      try {
        const data = await XamAuth.signIn({
          clientId: XAM_CLIENT_ID,
          redirectUri: XAM_REDIRECT_URI,
          scope: 'profile_basic'
        });
        await loginWithXamData(data);
      } catch (err) {
        const msg = err?.message || String(err);
        if (!/отмен|cancel/i.test(msg)) showToast(msg);
        console.error('XAM login error:', err);
      } finally {
        btn.disabled = false;
    ...
    const obs = new MutationObserver(syncPanelVisibility);
    const app = document.getElementById('app');
    if (authScreen) obs.observe(authScreen, { attributes: true, attributeFilter: ['class'] });
    if (app) obs.observe(app, { attributes: true, attributeFilter: ['class'] });
    syncPanelVisibility();
  }

  async function tryRestoreSession() {
    const session = loadXamSession();
    if (!session) return false;
    try {
      if (window.XamAuth?.fetchUser) {
        const fresh = await XamAuth.fetchUser(session.access_token);
        if (fresh?.id) session.user = fresh;
      }
      await loginWithXamData({
        access_token: session.access_token,
        expires_in: Math.max(60, Math.floor((session.expires_at - Date.now()) / 1000)),
        user: session.user
      });
      return true;
    } catch (err) {
      console.warn('XAM session restore failed:', err);
      clearXamSession();
      return false;
    }
  }
  ...
  async function boot() {
    ...
    // Redirect-flow (если вернулись с ?code=)
    if (window.XamAuth?.handleRedirect && new URLSearchParams(location.search).has('code')) {
      try {
        const data = await XamAuth.handleRedirect({
          clientId: XAM_CLIENT_ID,
          redirectUri: XAM_REDIRECT_URI
        });
        ...
        showToast(err?.message || 'Ошибка OAuth callback');
        console.error(err);
      }
    }
  ...
  if (document.readyState === 'loading') {
    ...
    // auth IIFE уже мог создать экран — чуть подождём XamAuth
    setTimeout(boot, 0);
  }
})();
...
(function() {
  ...
  style.textContent = `
    ...
    .chat-popover .popover-item:hover {
      background: var(--bg-elevated);
    }
    .chat-popover .popover-item:active {
      background: rgba(108, 99, 255, 0.08);
    }
    .chat-popover .popover-item .popover-icon {
      width: 22px;
      text-align: center;
      font-size: 16px;
      color: var(--text-secondary);
    ...
    .chat-popover .popover-item.danger {
      color: #E74C3C;
    }
    .chat-popover .popover-item.danger .popover-icon {
      color: #E74C3C;
    }
    .chat-popover .popover-divider {
      height: 1px;
      background: var(--border-light);
      margin: 4px 12px;
    }
    .chat-popover .popover-item .popover-badge {
      margin-left: auto;
      font-size: 11px;
      background: var(--bg-elevated);
      padding: 2px 8px;
    ...
    /* Модалка поиска */
    #modal-search-messages .search-results {
      max-height: 50vh;
      overflow-y: auto;
      margin-top: 8px;
    }
    #modal-search-messages .search-result-item {
      padding: 10px 12px;
      border-bottom: 1px solid var(--border-light);
      cursor: pointer;
      transition: background 0.15s;
    ...
    #modal-search-messages .search-result-item:hover {
      background: var(--bg-elevated);
    }
    #modal-search-messages .search-result-item .sr-sender {
      font-weight: 600;
      font-size: 13px;
      color: var(--primary);
    }
    #modal-search-messages .search-result-item .sr-text {
      font-size: 14px;
      color: var(--text-primary);
      word-break: break-word;
    }
    #modal-search-messages .search-result-item .sr-time {
      font-size: 11px;
      color: var(--text-hint);
      margin-top: 2px;
    }
    #modal-search-messages .search-input-wrap {
      display: flex;
      gap: 8px;
      align-items: center;
      background: var(--bg-elevated);
    ...
    #modal-search-messages .search-input-wrap:focus-within {
      border-color: var(--primary);
      background: var(--bg-surface);
    }
    #modal-search-messages .search-input-wrap input {
      flex: 1;
      border: none;
      background: none;
      padding: 10px 0;
    ...
    #modal-search-messages .search-input-wrap .search-icon {
      color: var(--text-hint);
    }
    #modal-search-messages .search-stats {
      font-size: 13px;
      color: var(--text-secondary);
      padding: 4px 0 8px;
    }
  ...
  // ─── 2. ДОБАВЛЯЕМ КНОПКУ В ШАПКУ ───
  function addChatMenuButton() {
    // Ждём, пока появится шапка чата
    const header = document.getElementById('chat-hdr');
    if (!header) {
      setTimeout(addChatMenuButton, 300);
      return;
    }

    // Не добавляем повторно
    if (document.getElementById('chat-menu-btn')) return;

    const btn = document.createElement('button');
    btn.id = 'chat-menu-btn';
    btn.className = 'hdr-btn';
    btn.innerHTML = '<i class="fas fa-ellipsis-v"></i>';
    btn.title = 'Меню чата';
    btn.setAttribute('aria-label', 'Меню чата');

    // Вставляем в конец шапки
    header.appendChild(btn);

    // Обработчик клика
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      toggleChatPopover(btn);
    });

    // При открытии/закрытии чата обновляем состояние кнопки (видимость)
    const origOpenChat = window.openChat;
    if (origOpenChat) {
      window.openChat = async function(chat) {
        await origOpenChat(chat);
        // Показываем кнопку только для активных чатов (не для списка)
  ...
  // ─── 3. СОЗДАЁМ ПОПОВЕР МЕНЮ ───
  let popover = null;
  let isPopoverOpen = false;

  function createChatPopover() {
  ...
  function buildPopoverItems() {
    const items = [
      { id: 'info', icon: 'fa-circle-info', label: 'Информация о чате' },
      { id: 'search', icon: 'fa-search', label: 'Поиск по сообщениям' },
      { id: 'pin', icon: 'fa-thumbtack', label: 'Закрепить чат' },
      { id: 'divider' },
      { id: 'export', icon: 'fa-download', label: 'Экспорт чата' },
      { id: 'notify', icon: 'fa-bell', label: 'Уведомления' },
      { id: 'divider' },
      { id: 'clear', icon: 'fa-eraser', label: 'Очистить историю', danger: true },
    ];

    // Для приватных чатов добавляем блокировку
    if (activeChat && activeChat.type === 'private') {
      const isBlocked = myProfile?.blockedUsers?.includes(activeChat.id) || false;
      items.push({
        id: 'block',
        icon: isBlocked ? 'fa-unlock' : 'fa-ban',
        label: isBlocked ? 'Разблокировать' : 'Заблокировать пользователя',
        danger: !isBlocked
      });
    }
  ...
  function renderPopoverItems() {
    if (!popover) return;
    const items = buildPopoverItems();
    popover.innerHTML = '';

    // Обновляем состояние закрепления для отображения
    const isPinned = isChatPinned(activeChat);
    ...
    items.forEach(item => {
      ...
      const btn = document.createElement('button');
      btn.className = 'popover-item' + (item.danger ? ' danger' : '');
      btn.setAttribute('role', 'menuitem');
      btn.dataset.action = item.id;

      // Для закрепления меняем текст в зависимости от состояния
      ...
      btn.innerHTML = `
        <span class="popover-icon"><i class="fas ${item.icon}"></i></span>
        <span>${label}</span>
        ${item.id === 'pin' ? `<span class="popover-badge">${isPinned ? '✓' : ''}</span>` : ''}
        ${item.id === 'notify' ? `<span class="popover-badge">${isChatMuted(activeChat) ? '🔇' : '🔔'}</span>` : ''}
      `;

      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        closePopover();
        handlePopoverAction(item.id);
      });
  ...
  // ─── 4. УПРАВЛЕНИЕ ПОПОВЕРОМ ───
  function toggleChatPopover(anchorBtn) {
    if (isPopoverOpen) {
      closePopover();
      return;
    }

    if (!popover) createChatPopover();
    renderPopoverItems();

    // Позиционируем
    const rect = anchorBtn.getBoundingClientRect();
    const popoverWidth = 240;
    let left = rect.right - popoverWidth + 8;
    let top = rect.bottom + 6;

    // Корректируем, чтобы не выходило за экран
    if (left < 8) left = 8;
    ...
    // Задержка, чтобы не закрыть сразу при открытии
    setTimeout(() => {
    ...
    document.addEventListener('scroll', closeOnScroll, { passive: true, once: true });
  ...
  async function handlePopoverAction(action) {
    if (!activeChat || !me) {
    ...
    switch (action) {
      case 'info':
        ...
        if (activeChat.type === 'private') {
          ...
          const user = usersCache.get(uid);
          if (user) {
            if (typeof openUserView === 'function') {
              openUserView(uid);
            } else {
              showToast('Функция профиля недоступна');
            }
          } else {
            showToast('Пользователь не найден');
          }
        } else if (activeChat.type === 'group' || activeChat.type === 'channel') {
          if (typeof openGroupInfo === 'function') {
            openGroupInfo(activeChat);
          } else {
            showToast('Функция информации о группе недоступна');
          }
        } else {
          showToast('Информация недоступна');
        }
        break;
      ...
      case 'pin':
        await togglePinChat(activeChat);
        break;

      case 'export':
        await exportChat(activeChat);
        break;

      case 'notify':
        await toggleMuteChat(activeChat);
        break;

      case 'clear':
        if (confirm(`Очистить всю историю чата "${activeChat.name}"?`)) {
          await clearChatHistory(activeChat);
        }
        break;
      ...
      case 'block':
        if (activeChat.type === 'private') {
          ...
          const confirmMsg = isBlocked
            ? `Разблокировать пользователя ${activeChat.name}?`
            : `Заблокировать пользователя ${activeChat.name}? Он не сможет писать вам.`;
          if (confirm(confirmMsg)) {
            await toggleBlockUser(activeChat.id);
            showToast(isBlocked ? 'Пользователь разблокирован' : 'Пользователь заблокирован');
          }
        }
        break;
  ...
  // 6.1 Закрепление чата (храним в Firestore)
  async function togglePinChat(chat) {
    if (!me) return;
    const key = chat.type === 'private' ? `private_${chat.id}` : `room_${chat.id}`;
    const pinned = myProfile?.pinnedChats || [];

    let newPinned;
    if (pinned.includes(key)) {
      newPinned = pinned.filter(k => k !== key);
      showToast('Чат откреплён');
    } else {
      newPinned = [...pinned, key];
      showToast('Чат закреплён');
    }

    await setDoc(doc(db, 'users', me.uid), { pinnedChats: newPinned }, { merge: true });
    if (myProfile) myProfile.pinnedChats = newPinned;

    // Обновляем список чатов
    if (typeof buildChatList === 'function') {
      await buildChatList();
      renderChatList(document.getElementById('chat-search')?.value || '');
    }
  }
  ...
  function isChatPinned(chat) {
    if (!myProfile?.pinnedChats) return false;
    const key = chat.type === 'private' ? `private_${chat.id}` : `room_${chat.id}`;
    return myProfile.pinnedChats.includes(key);
  }

  // 6.2 Уведомления (mute) — храним в Firestore
  async function toggleMuteChat(chat) {
    if (!me) return;
    const key = chat.type === 'private' ? `private_${chat.id}` : `room_${chat.id}`;
    const muted = myProfile?.mutedChats || [];

    let newMuted;
    if (muted.includes(key)) {
      newMuted = muted.filter(k => k !== key);
    ...
    await setDoc(doc(db, 'users', me.uid), { mutedChats: newMuted }, { merge: true });
    if (myProfile) myProfile.mutedChats = newMuted;
  }

  function isChatMuted(chat) {
    if (!myProfile?.mutedChats) return false;
    const key = chat.type === 'private' ? `private_${chat.id}` : `room_${chat.id}`;
    return myProfile.mutedChats.includes(key);
  }

  // 6.3 Очистка истории
  async function clearChatHistory(chat) {
    if (!me) return;
    const chatId = await getChatId(chat);
    if (!chatId) return;
    ...
    const snap = await getDocs(query(collection(db, 'messages'), where('chatId', '==', chatId)));
    if (snap.empty) {
      showToast('В чате нет сообщений');
      return;
    }
  ...
  async function exportChat(chat) {
    ...
    // Формируем текст
    let txt = `=== Экспорт чата "${chat.name}" ===\n`;
    txt += `Дата: ${new Date().toLocaleString()}\n`;
    txt += `Тип: ${chat.type === 'private' ? 'Приватный' : chat.type === 'group' ? 'Группа' : 'Канал'}\n`;
    txt += `Всего сообщений: ${msgs.length}\n`;
    txt += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;

    for (const msg of msgs) {
      const date = msg.timestamp?.toDate?.() || new Date();
      const time = date.toLocaleString();
      const sender = msg.senderUid === me.uid ? 'Вы' : getDisplayName(msg.senderUid);
      const text = msg.isDeleted ? '[Удалено]' : (msg.text || '[Пусто]');
    ...
    a.href = url;
    a.download = `chat_${chat.name}_${new Date().toISOString().slice(0,10)}.txt`;
  ...
  // 6.5 Блокировка пользователя (обёртка над существующей функцией)
  async function toggleBlockUser(uid) {
    ...
    let newBlocked;
    if (isBlocked) {
      newBlocked = blocked.filter(u => u !== uid);
    } else {
      newBlocked = [...blocked, uid];
    }

    await setDoc(doc(db, 'users', me.uid), { blockedUsers: newBlocked }, { merge: true });
    if (myProfile) myProfile.blockedUsers = newBlocked;

    // Если чат открыт и это приватный чат, обновляем статус
    if (activeChat && activeChat.type === 'private' && activeChat.id === uid) {
      if (typeof updateChatHeaderWithProfile === 'function') {
        updateChatHeaderWithProfile();
      }
    }
  }

  // ─── 7. ПОИСК ПО СООБЩЕНИЯМ (МОДАЛКА) ───
  function openSearchModal() {
    // Создаём модалку, если её нет
    let modal = document.getElementById('modal-search-messages');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'modal-search-messages';
      modal.className = 'modal-ov';
      modal.innerHTML = `
        <div class="modal-sheet">
          <div class="modal-pill"></div>
          ...
          <div class="modal-body">
            <div class="search-input-wrap">
              ...
              <input type="text" id="search-query-input" placeholder="Введите текст для поиска..." autofocus>
              <button id="search-clear-btn" style="background:none;border:none;color:var(--text-hint);font-size:16px;display:none;">
      ...
      // Закрытие
      modal.querySelector('.modal-close').addEventListener('click', () => closeModal('modal-search-messages'));
      modal.addEventListener('click', (e) => {
        if (e.target === modal) closeModal('modal-search-messages');
      });
      ...
      input.addEventListener('input', async () => {
        ...
        if (!query || query.length < 2) {
          resultsEl.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-hint);">Введите минимум 2 символа</div>';
          statsEl.textContent = '';
          return;
        }

        // Ищем в allMessages (уже загруженных)
        if (!allMessages || allMessages.length === 0) {
          resultsEl.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-hint);">Нет сообщений для поиска</div>';
          statsEl.textContent = '';
          return;
        }
        ...
        statsEl.textContent = `Найдено: ${results.length} сообщений`;

        if (results.length === 0) {
          resultsEl.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-hint);">Ничего не найдено</div>';
          return;
        }

        // Показываем результаты (последние 50)
        const show = results.slice(-50).reverse();
        resultsEl.innerHTML = show.map(msg => {
          const sender = msg.senderUid === me.uid ? 'Вы' : getDisplayName(msg.senderUid);
          ...
          if (idx !== -1) {
            ...
            text = `${before}<mark style="background:var(--primary-light);color:var(--text-primary);padding:0 2px;border-radius:3px;">${match}</mark>${after}`;
          }
          return `
            <div class="search-result-item" data-msg-id="${msg.id}">
              <div class="sr-sender">${esc(sender)}</div>
              <div class="sr-text">${text}</div>
              <div class="sr-time">${esc(time)}</div>
            </div>
          `;
        }).join('');

        // Клик по результату — прокручиваем к сообщению
        resultsEl.querySelectorAll('.search-result-item').forEach(el => {
          el.addEventListener('click', () => {
            ...
            if (bubble) {
              closeModal('modal-search-messages');
              bubble.scrollIntoView({ behavior: 'smooth', block: 'center' });
              // Подсветка
              bubble.style.transition = 'background 0.6s';
              bubble.style.background = 'rgba(108,99,255,0.15)';
              setTimeout(() => {
                bubble.style.background = '';
              }, 1200);
            } else {
              showToast('Сообщение не найдено в текущем списке');
              closeModal('modal-search-messages');
            }
          });
        });
      ...
      // Enter для поиска (уже работает через input)
    }

    // Открываем модалку и фокусируем поле
    openModal('modal-search-messages');
    setTimeout(() => {
      const input = document.getElementById('search-query-input');
      if (input) {
        input.focus();
  ...
  function initChatMenu() {
    ...
    // Создаём popover заранее
    createChatPopover();

    // При смене чата обновляем меню (кнопка уже есть)
    const origOpenChat = window.openChat;
    if (origOpenChat) {
      window.openChat = async function(chat) {
        await origOpenChat(chat);
        // Закрываем popover при смене чата
        closePopover();
        // Обновляем содержимое при следующем открытии
      };
    }

    console.log('✅ Кнопка «три точки» в шапке чата активирована');
  }

  // Запускаем после загрузки DOM
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initChatMenu);
  } else {
    initChatMenu();
  }

  // ─── 9. ДОПОЛНИТЕЛЬНО: ОБНОВЛЕНИЕ МЕНЮ ПРИ ИЗМЕНЕНИИ СОСТОЯНИЯ ───
  // Переопределяем closePopover для очистки
  ...
  // При клике на другие элементы закрываем
  document.addEventListener('click', (e) => {
    if (isPopoverOpen && popover && !popover.contains(e.target)) {
...
// ════════════════════════════════════════════════════
// ПОИСК ПОЛЬЗОВАТЕЛЕЙ И СООБЩЕСТВ (ГРУПП / КАНАЛОВ)
// ════════════════════════════════════════════════════
$('contacts-search').oninput = async function () {
  const q = this.value.trim().toLowerCase();
  const res = $('contacts-search-res');
  ...
  // 2. Поиск среди сообществ (группы и каналы)
  const foundRooms = [];
  roomsCache.forEach((r, id) => {
    const nm = (r.name || '').toLowerCase();
    const un = (r.username || '').toLowerCase();
  ...
  if (foundRooms.length) {
    ...
    foundRooms.slice(0, 10).forEach(r => {
      ...
      const badge = r.type === 'channel' ? '📢 Канал' : '👥 Группа';
      html += `
        <div class="contact-item" data-room-id="${r.id}">
          <div class="ci-av">${avatarHtml(r.avatar || (r.type === 'channel' ? '📢' : '👥'))}</div>
          <div class="ci-info">
            <div class="ci-name">${esc(r.name)}</div>
            <div class="ci-uname">${esc(r.username || '')} <span style="opacity:0.6;">• ${badge}</span></div>
          ...
          <div class="ci-actions">
            <button class="ci-btn ${isMember ? '' : 'green'}" data-room-id="${r.id}" data-action="room-join">
              ${isMember ? '💬' : '+'}
            </button>
          </div>
        </div>`;
  ...
  // Слушатели событий: Пользователи
  res.querySelectorAll('.contact-item[data-uid]').forEach(div => {
    const uid = div.dataset.uid;
    div.querySelectorAll('.ci-btn').forEach(b => b.onclick = async e => {
      e.stopPropagation();
      const action = b.dataset.action;
      if (action === 'msg') {
        const chat = chatList.find(c => c.id === uid && c.type === 'private') || { id: uid, name: getDisplayName(uid), avatar: usersCache.get(uid)?.avatar, type: 'private' };
        openChat(chat);
      } else if (action === 'add') {
        await setDoc(doc(db, 'users', me.uid), { contacts: arrayUnion(uid) }, { merge: true });
        if (myProfile) myProfile.contacts = [...(myProfile.contacts || []), uid];
  ...
  res.querySelectorAll('.contact-item[data-room-id]').forEach(div => {
    ...
    div.querySelectorAll('.ci-btn').forEach(b => b.onclick = async e => {
      e.stopPropagation(); // Блокируем клик по самой карточке, чтобы не было двойного срабатывания
      
      const isMember = room.members?.includes(me.uid) || room.subscribers?.includes(me.uid);
      
      // Если пользователь не состоит в группе/канале — автоматически вступаем
      if (!isMember) {
        try {
          const field = room.type === 'channel' ? 'subscribers' : 'members';
          
          // Визуально сразу делаем кнопку "чатом", не дожидаясь ответа сервера
          b.classList.remove('green');
          b.innerText = '💬';

          await updateDoc(doc(db, 'rooms', roomId), { [field]: arrayUnion(me.uid) });
          room[field] = [...(room[field] || []), me.uid];
          
          if (typeof showToast === 'function') showToast(`✅ Вы вступили в "${room.name}"`);
        } catch (error) {
          console.error("Ошибка при вступлении в сообщество:", error);
          // Если возникла ошибка (например, нет прав), возвращаем кнопку обратно
          b.classList.add('green');
          b.innerText = '+';
          return; // Прерываем выполнение, если не удалось вступить
        }
      }
      
      // ВАЖНО: передаем id: roomId, так как room.id может быть undefined
      openChat({ 
        id: roomId, 
        name: room.name, 
        avatar: room.avatar, 
    ...
    // Клик по всей строке (помимо кнопки)
    div.onclick = e => {
      if (e.target.closest('.ci-btn')) return; // Игнорируем, если кликнули по кнопке
      
      // ВАЖНО: передаем id: roomId
      openChat({ 
        id: roomId, 
        name: room.name, 
        avatar: room.avatar, 
...
// ════════════════════════════════════════════════════
//  РАЗДЕЛ «ДЛЯ РАЗРАБОТЧИКОВ» (Bots, OAuth, Docs)
// ════════════════════════════════════════════════════
...
(function() {
  ...
  // ─── ПОКАЗАТЬ ПОРТАЛ РАЗРАБОТЧИКА ─────────────────
  function showDeveloperPortal() {
    ...
    if (backBtn) {
      backBtn._origOnclick = backBtn.onclick;
      backBtn.onclick = () => {
        if (body.dataset.prev) {
          body.innerHTML = body.dataset.prev;
          delete body.dataset.prev;
          // Восстанавливаем обработчик
          if (backBtn._origOnclick) {
            backBtn.onclick = backBtn._origOnclick;
          } else {
            backBtn.onclick = null;
          }
          // Перерендерим настройки (чтобы восстановить пункты)
          renderSettings();
        }
      };
    }
    ...
    body.innerHTML = `
      <div class="dev-portal">
        <div class="dev-tabs">
          <button class="dev-tab active" data-tab="bots">🤖 Боты</button>
          <button class="dev-tab" data-tab="oauth">🔑 OAuth</button>
          <button class="dev-tab" data-tab="docs">📄 Документация</button>
        </div>
        <div class="dev-tab-content active" id="dev-tab-bots"></div>
        <div class="dev-tab-content" id="dev-tab-oauth"></div>
        <div class="dev-tab-content" id="dev-tab-docs"></div>
      </div>
    `;
    ...
    body.querySelectorAll('.dev-tab').forEach(tab => {
      tab.onclick = () => {
        body.querySelectorAll('.dev-tab').forEach(t => t.classList.remove('active'));
        tab.classList.add('active');
        body.querySelectorAll('.dev-tab-content').forEach(c => c.classList.remove('active'));
        const target = document.getElementById('dev-tab-' + tab.dataset.tab);
        if (target) target.classList.add('active');
        // Загружаем данные
        if (tab.dataset.tab === 'bots') renderBotsTab();
        else if (tab.dataset.tab === 'oauth') renderOAuthTab();
        else if (tab.dataset.tab === 'docs') renderDocsTab();
      };
    });
  ...
  async function renderBotsTab() {
    const container = document.getElementById('dev-tab-bots');
    if (!container) return;
    container.innerHTML = '<div class="center-loader"><div class="loader"></div></div>';
    try {
      ...
      let html = `
        <div class="dev-section-hdr">
          <h4>Ваши боты</h4>
          <button class="dev-add-btn" id="create-bot-btn">+ Создать бота</button>
        </div>
      `;
      if (bots.length === 0) {
        html += `<div style="padding:20px;text-align:center;color:var(--text-hint);">У вас нет ботов. Создайте первого!</div>`;
      } else {
          ...
          html += `
            <div class="dev-card">
              <div class="dev-card-title">
                <span>${esc(bot.name)}</span>
                <span>
                  <button class="dev-settings-btn" data-bot-id="${bot.id}" title="Настроить">⚙️</button>
                  <button class="dev-del-btn" data-bot-id="${bot.id}" title="Удалить">🗑️</button>
                </span>
              </div>
              <div class="dev-card-field">@${esc(bot.username)}</div>
              <div class="dev-card-field">Токен: <code>${esc(bot.token)}</code></div>
              <div class="dev-card-field">Статус: ${bot.enabled ? '🟢 Активен' : '🔴 Неактивен'}</div>
            </div>
          `;
        });
        html += `</div>`;
      ...
      container.querySelector('#create-bot-btn')?.addEventListener('click', openCreateBotModal);

      // Настройка бота
      container.querySelectorAll('.dev-settings-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const botId = btn.dataset.botId;
          const bot = bots.find(b => b.id === botId);
          if (bot) openBotSettingsModal(bot);
        });
      });
      ...
      container.querySelectorAll('.dev-del-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          if (confirm('Удалить этого бота?')) {
            ...
            await deleteDoc(doc(db, 'bot_settings', botId));
      ...
      container.innerHTML = `<div style="padding:20px;text-align:center;color:var(--red);">Ошибка загрузки ботов</div>`;
    }
  }

  // ─── МОДАЛКА СОЗДАНИЯ БОТА ────────────────────────
  function openCreateBotModal() {
    let modal = document.getElementById('modal-create-bot');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'modal-create-bot';
      modal.className = 'modal-ov';
      modal.innerHTML = `
        <div class="modal-sheet">
          <div class="modal-pill"></div>
          <div class="modal-hdr"><h3>Создать бота</h3><button class="modal-close">✕</button></div>
          <div class="modal-body">
            <div class="modal-field"><label>Имя бота</label><input type="text" id="bot-name" placeholder="Мой бот"></div>
            <div class="modal-field"><label>@юзернейм</label><input type="text" id="bot-username" placeholder="@mybot" autocapitalize="none" autocorrect="off"></div>
            <button class="modal-btn" id="create-bot-submit">Создать</button>
          </div>
        </div>
      `;
      document.body.appendChild(modal);
      modal.querySelector('.modal-close').onclick = () => closeModal('modal-create-bot');
      modal.addEventListener('click', (e) => { if (e.target === modal) closeModal('modal-create-bot'); });
      document.getElementById('create-bot-submit').onclick = async () => {
        ...
        if (!username.startsWith('@')) { showToast('Юзернейм должен начинаться с @'); return; }
        // Проверка уникальности
        const q = query(collection(db, 'bots'), where('username', '==', username));
        const snap = await getDocs(q);
        if (!snap.empty) { showToast('Юзернейм уже занят'); return; }
        const token = 'bot_' + btoa(Math.random().toString(36) + Date.now().toString(36)).slice(0, 20);
        const botData = {
          name,
          username,
          token,
        ...
        await setDoc(doc(db, 'bot_settings', docRef.id), {
          botUid: docRef.id,
          ownerUid: me.uid,
          rules: [],
          defaultResponse: '',
  ...
  // ─── МОДАЛКА НАСТРОЙКИ БОТА (правила) ─────────────
  async function openBotSettingsModal(bot) {
    const settingsSnap = await getDoc(doc(db, 'bot_settings', bot.id));
    let settings = settingsSnap.exists() ? settingsSnap.data() : { rules: [], defaultResponse: '', enabled: false };

    let modal = document.getElementById('modal-bot-settings');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'modal-bot-settings';
      modal.className = 'modal-ov';
      modal.innerHTML = `
        <div class="modal-sheet">
          <div class="modal-pill"></div>
          <div class="modal-hdr"><h3 id="bot-settings-title">Настройка бота</h3><button class="modal-close">✕</button></div>
          <div class="modal-body" id="bot-settings-body"></div>
        </div>
      `;
      document.body.appendChild(modal);
      modal.querySelector('.modal-close').onclick = () => closeModal('modal-bot-settings');
      modal.addEventListener('click', (e) => { if (e.target === modal) closeModal('modal-bot-settings'); });
    }

    document.getElementById('bot-settings-title').textContent = `Настройка бота: @${bot.username}`;
    const body = document.getElementById('bot-settings-body');

    let rules = settings.rules || [];
    if (rules.length === 0) rules = [{ trigger: '', response: '' }, { trigger: '', response: '' }];

    function renderRules() {
      body.innerHTML = `
        <div class="modal-field" style="display:flex;align-items:center;gap:12px;">
          <label style="margin:0;">Бот активен</label>
          <input type="checkbox" id="bot-enabled" ${settings.enabled ? 'checked' : ''}>
        </div>
        <div class="modal-field">
          <label>Правила (ключевое слово → ответ)</label>
          <div id="rules-container"></div>
          <button class="dev-add-btn" id="add-rule-btn" style="margin-top:6px;">+ Добавить правило</button>
        </div>
        <div class="modal-field">
          <label>Ответ по умолчанию</label>
          <input type="text" id="default-response" value="${esc(settings.defaultResponse || '')}" placeholder="Ответ, если ни одно правило не сработало">
        </div>
        <button class="modal-btn" id="save-bot-settings">Сохранить</button>
      `;
      ...
      rules.forEach((rule, index) => {
        ...
        row.innerHTML = `
          <input type="text" class="rule-trigger" value="${esc(rule.trigger || '')}" placeholder="Ключевое слово" data-index="${index}">
          <input type="text" class="rule-response" value="${esc(rule.response || '')}" placeholder="Ответ" data-index="${index}">
          <button class="rule-del" data-index="${index}">✕</button>
        `;
        container.appendChild(row);
      });

      // Удаление правила
      container.querySelectorAll('.rule-del').forEach(btn => {
        btn.addEventListener('click', () => {
      ...
      document.getElementById('save-bot-settings').addEventListener('click', async () => {
        ...
        const responses = document.querySelectorAll('.rule-response');
        for (let i = 0; i < triggers.length; i++) {
          const t = triggers[i].value.trim();
          const r = responses[i].value.trim();
          if (t || r) {
            newRules.push({ trigger: t, response: r });
          }
        }
        await setDoc(doc(db, 'bot_settings', bot.id), {
          botUid: bot.id,
          ownerUid: me.uid,
          rules: newRules,
          defaultResponse,
        ...
        // Обновляем статус в коллекции bots
        await updateDoc(doc(db, 'bots', bot.id), { enabled });
        showToast('Настройки сохранены');
        closeModal('modal-bot-settings');
        renderBotsTab();
      });
    }

    renderRules();
    openModal('modal-bot-settings');
  }

  // ─── ВКЛАДКА «OAUTH» ───────────────────────────────
  async function renderOAuthTab() {
    const container = document.getElementById('dev-tab-oauth');
    if (!container) return;
    container.innerHTML = '<div class="center-loader"><div class="loader"></div></div>';
    try {
      ...
      let html = `
        <div class="dev-section-hdr">
          <h4>OAuth приложения</h4>
          <button class="dev-add-btn" id="create-oauth-btn">+ Создать приложение</button>
        </div>
      `;
      if (apps.length === 0) {
        html += `<div style="padding:20px;text-align:center;color:var(--text-hint);">Нет OAuth приложений</div>`;
      } else {
        html += `<div class="dev-list">`;
        apps.forEach(app => {
          html += `
            <div class="dev-card">
              <div class="dev-card-title">
                ...
                <button class="dev-del-btn" data-app-id="${app.id}">🗑️</button>
              </div>
              ...
              <div class="dev-card-field">Redirect URI: ${esc(app.redirectUri)}</div>
            </div>
          `;
        });
        html += `</div>`;
      ...
      container.querySelector('#create-oauth-btn')?.addEventListener('click', openCreateOAuthModal);
      container.querySelectorAll('.dev-del-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
          const appId = btn.dataset.appId;
          if (confirm('Удалить OAuth приложение?')) {
            await deleteDoc(doc(db, 'oauth_apps', appId));
      ...
      container.innerHTML = `<div style="padding:20px;text-align:center;color:var(--red);">Ошибка загрузки</div>`;
    }
  }

  // ─── МОДАЛКА СОЗДАНИЯ OAuth ПРИЛОЖЕНИЯ ────────────
  function openCreateOAuthModal() {
    let modal = document.getElementById('modal-create-oauth');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'modal-create-oauth';
      modal.className = 'modal-ov';
      modal.innerHTML = `
        <div class="modal-sheet">
          <div class="modal-pill"></div>
          <div class="modal-hdr"><h3>Создать OAuth приложение</h3><button class="modal-close">✕</button></div>
          <div class="modal-body">
            <div class="modal-field"><label>Название</label><input type="text" id="oauth-name" placeholder="Моё приложение"></div>
            <div class="modal-field"><label>Redirect URI</label><input type="text" id="oauth-redirect" placeholder="https://example.com/callback"></div>
            <button class="modal-btn" id="create-oauth-submit">Создать</button>
          </div>
        </div>
      `;
      document.body.appendChild(modal);
      modal.querySelector('.modal-close').onclick = () => closeModal('modal-create-oauth');
      modal.addEventListener('click', (e) => { if (e.target === modal) closeModal('modal-create-oauth'); });
      document.getElementById('create-oauth-submit').onclick = async () => {
        const name = document.getElementById('oauth-name').value.trim();
        const redirect = document.getElementById('oauth-redirect').value.trim();
        if (!name || !redirect) { showToast('Заполните все поля'); return; }
        const clientId = 'client_' + btoa(Math.random().toString(36) + Date.now().toString(36)).slice(0, 20);
        const clientSecret = 'secret_' + btoa(Math.random().toString(36) + Date.now().toString(36)).slice(0, 20);
  ...
  function renderDocsTab() {
    const container = document.getElementById('dev-tab-docs');
    if (!container) return;
    container.innerHTML = `
      <div class="dev-docs" style="padding:12px 0;">
        <h3 style="font-weight:700;font-size:18px;margin-bottom:12px;">Документация по API ботов</h3>
        <p>Боты в NexLink позволяют автоматизировать ответы на сообщения. Для работы с ботом используйте HTTP API.</p>

        <h4 style="margin:16px 0 8px;">Как получить токен бота</h4>
        <p>Токен бота генерируется автоматически при создании бота и отображается в списке ботов в разделе «Для разработчиков».</p>

        <h4 style="margin:16px 0 8px;">Отправка сообщения через API</h4>
        <p>Отправьте POST-запрос на эндпоинт:</p>
        <pre><code>POST /api/v1/bot/send</code></pre>
        <p>Заголовки:</p>
        <pre><code>Authorization: Bearer {токен_бота}
        ...
        <p>Пример с curl:</p>
        <pre><code>curl -X POST https://nexlink.example.com/api/v1/bot/send \\
          -H "Authorization: Bearer bot_abc123" \\
          -H "Content-Type: application/json" \\
          -d '{"chatId":"chat_123","text":"Привет!"}'</code></pre>
        <p>Пример на Python:</p>
        <pre><code>import requests

        url = "https://nexlink.example.com/api/v1/bot/send"
        headers = {
          "Authorization": "Bearer bot_abc123",
        ...
        <h4 style="margin:16px 0 8px;">Полная документация</h4>
        <p>Подробнее об API ботов читайте в <a href="#" style="color:var(--primary);">официальной документации</a> (ссылка временно не активна).</p>
      </div>
    `;
  }

  // ─── ДОБАВЛЯЕМ ПУНКТ В ГЛАВНОЕ МЕНЮ НАСТРОЕК ──────
  // Переопределяем renderSettings, чтобы добавить пункт «Для разработчиков»
  const origRenderSettings = window.renderSettings;
  if (typeof origRenderSettings === 'function') {
    window.renderSettings = function() {
      origRenderSettings.apply(this, arguments);
      // Добавляем пункт в раздел «Дополнительно»
      const body = document.getElementById('settings-body');
      if (!body) return;
      let targetSection = null;
      ...
      if (!targetSection) return;
      // Не добавляем повторно
      if (targetSection.querySelector('#s-developer')) return;
      const item = document.createElement('div');
      item.className = 'settings-item';
      item.id = 's-developer';
      item.innerHTML = `<div class="si-icon">🛠️</div><div class="si-text">Для разработчиков</div><span class="si-arrow">›</span>`;
      item.onclick = showDeveloperPortal;
      targetSection.appendChild(item);
    };
  }

  // ─── ЭКСПОРТ ГЛОБАЛЬНЫХ ФУНКЦИЙ (для доступа извне) ──
  window.showDeveloperPortal = showDeveloperPortal;
  window.renderBotsTab = renderBotsTab;
  window.renderOAuthTab = renderOAuthTab;
  ...
  window.openCreateOAuthModal = openCreateOAuthModal;

  console.log('✅ Раздел «Для разработчиков» активирован');
})();
